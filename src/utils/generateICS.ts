...
export function generateICSContent(events: CalendarEvent[], calendarName?: string): string {
  ...
  events.forEach((event, index) => {
    ...
    if (event.interpretation && typeof event.interpretation === 'object') {
      ...
      descParts.push('');
      // V2: que_se_activa, como_se_siente, consejo, advertencias, mantra
      if (interp.que_se_activa) descParts.push(`🔥 Qué se activa: ${interp.que_se_activa}`);
      if (interp.como_se_siente) descParts.push(`🧠 Cómo se siente: ${interp.como_se_siente}`);
      if (interp.consejo) descParts.push(`🔧 Consejo: ${interp.consejo}`);
      if (interp.advertencias) descParts.push(`⚠️ Advertencias: ${interp.advertencias}`);
      if (interp.mantra) descParts.push(`✨ Mantra: ${interp.mantra}`);
