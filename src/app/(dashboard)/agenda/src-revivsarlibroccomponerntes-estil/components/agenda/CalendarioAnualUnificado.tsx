...
import { Calendar, TrendingUp, Star, Sparkles, Moon, Sun } from 'lucide-react';
import { FooterLibro } from './MesCompleto';

// ============ LÍNEA DE TIEMPO EMOCIONAL - CON ESTILOS ============
export const LineaTiempoEmocionalEstilizada: React.FC<{
  startDate: Date;
  endDate: Date;
}> = ({ startDate, endDate }) => {
  ...
  return (
    <div className={`print-page bg-white flex flex-col relative ${config.pattern}`} style={{ padding: '15mm' }}>
      ...
      <div className="text-center mb-6">
        ...
        <h2 className={`font-display text-2xl ${config.titleGradient}`}>Línea del Tiempo Emocional</h2>
      ...
      <div className="flex-1 grid grid-cols-4 gap-3 mb-6">
        {months.map((month, index) => (
          <div 
            key={index}
            className={`${config.highlightSecondary} p-3 flex flex-col`}
          >
            ...
            <div className="flex gap-1 mb-2">
              {[1, 2, 3, 4, 5].map(i => (
                <div key={i} className={`w-4 h-4 border ${config.cardBorder} bg-white`} />
              ))}
            </div>
            {/* Espacio para notas */}
      ...
      <div className={`${config.highlightPrimary} p-4`}>
        ...
        <div className="flex items-center justify-around text-xs">
          <div className="flex items-center gap-2">
            <div className={`w-5 h-5 ${config.highlightSecondary}`} />
            <span className="text-gray-600">Calma</span>
          </div>
          <div className="flex items-center gap-2">
            <div className={`w-5 h-5 ${config.badgeSecondary}`} />
            <span className="text-gray-600">Movimiento</span>
          </div>
          <div className="flex items-center gap-2">
            <div className={`w-5 h-5 ${config.badgePrimary}`} />
...
export const MesesClavePuntosGiroEstilizado: React.FC = () => {
  const { config } = useStyle();
  ...
  return (
    <div className={`print-page bg-white flex flex-col relative ${config.pattern}`} style={{ padding: '15mm' }}>
      ...
      <div className="text-center mb-6">
        ...
        <h2 className={`font-display text-2xl ${config.titleGradient}`}>Meses Clave y Puntos de Giro</h2>
        <div className={`${config.divider} w-24 mx-auto mt-4`} />
      ...
      <div className="flex-1 space-y-4">
        {[1, 2, 3, 4].map((num) => (
          <div key={num} className={`${config.highlightSecondary} p-4`}>
            <div className="flex items-center gap-3 mb-3">
              <span className={`${config.iconPrimary} font-display text-xl font-bold`}>{num}.</span>
...
export const GrandesAprendizajesEstilizado: React.FC = () => {
  const { config } = useStyle();
  ...
  return (
    <div className={`print-page bg-white flex flex-col relative ${config.pattern}`} style={{ padding: '15mm' }}>
      ...
      <div className="text-center mb-6">
        ...
        <h2 className={`font-display text-2xl ${config.titleGradient}`}>Grandes Aprendizajes del Ciclo</h2>
        <div className={`${config.divider} w-24 mx-auto mt-4`} />
        <p className={`text-sm italic ${config.iconSecondary} mt-4`}>
      ...
      {/* Cita */}
      <div className={`${config.headerBg} p-4 text-center mt-4`}>
...
export default LineaTiempoEmocionalEstilizada;
