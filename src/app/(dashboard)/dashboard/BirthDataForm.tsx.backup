...
export default function BirthDataForm() {
      ...
      try {
        ...
        if (response.ok) {
          ...
          if (data) {
            ...
            // Si no hay datos pero tenemos el displayName, usarlo
            if (user.displayName) {
        ...
        // Si hay error pero tenemos el displayName, usarlo
        if (user.displayName) {
          setValue('fullName', user.displayName);
        }
      } finally {
  ...
  const onSubmit = async (data: BirthDataFormValues) => {
    ...
    try {
      ...
      // Intentar generar la carta natal
      try {
    ...
    <div className="max-w-md w-full mx-auto bg-white p-8 rounded-lg shadow-md">
      <div>
        ...
        <p className="text-sm text-gray-600 mb-6">
          Estos datos son necesarios para generar tu carta natal personalizada y crear tu agenda astrológica.
        </p>
      </div>
